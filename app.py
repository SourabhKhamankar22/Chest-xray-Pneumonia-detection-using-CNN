...
with col1:
    ...
    st.write("or choose a sample image from the project test folder (optional)")

    sample_dir = Path("chest_xray_dataset")  # try to offer sample images (if present)

    sample_images = []

    if sample_dir.exists():

        # find a few jpg/png under the test folder

        for p in sample_dir.rglob("*"):

            # VITAL FIX 1: Skip __MACOSX metadata folders and files

            if "__MACOSX" in str(p):

                continue

            # VITAL FIX 2: Ensure we are only looking for files, not directories or system files

            if p.is_file() and p.suffix.lower() in (".jpg", ".jpeg", ".png") and "test" in str(p).lower():

                sample_images.append(p)

        sample_images = sorted(sample_images)[:6]

    if sample_images:

        # Use p.name for the display, but store the full path in the list

        sample_names = [p.name for p in sample_images]

        sample_choice = st.selectbox("Pick a sample image", ["-- none --"] + sample_names)

        sample_path = None

        if sample_choice != "-- none --":

            # Map the selected name back to the full path object

            sample_path = next((p for p in sample_images if p.name == sample_choice), None)
...
if uploaded:
    ...
    # VITAL FIX 3: Open the image using the full path object stored in sample_path

    try:

        # Convert Path object to string for Image.open if needed, then open

        pil_image = Image.open(str(sample_path)) 

    except Exception as e:

        # Added error handling for sample images too
...
if pil_image is None:
    ...
    st.info("Upload an image or select a sample to see predictions.")
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name=None):
    ...
    # VITAL FIX 4: Convert the input NumPy array back to a TensorFlow Tensor
